...
import urllib.request, datetime, pytz, json, telegram, time, re, sys, mariadb
...
if __name__ == '__main__':
    ...
    # print('Result: ', result)
    # 출력 형태 : {'POP': '20', 'PTY': '0', 'REH': '95', 'SKY': '3', 'T3H': '14', 'UUU': '0.4', 'VEC': '214', 'VVV': '0.6', 'WSD': '0.7'}
...
# 구할 건 강수확률POP, 습도REH, 하늘상태SKY, 강수형태PTY
msg = []
ma  = msg.append
...
short = DataFrame(msg).values    # 3, 1, 2, 0 순서로 실행하면 날씨 읽어보기에 좋다.
msg = short[0] + short[4] + short[2] + short[3] + short[1]
...
# DB와 Table 생성 
  ...
  #                                                           T3H VARCHAR(255), UUU VARCHAR(255), VEC VARCHAR(255), VVV VARCHAR(255), WSD VARCHAR(255))")
...
# | WSD       | varchar(255) | YES  |     | NULL    |       |
# +-----------+--------------+------+-----+---------+-------+
# +-------------+------+------+------+------+------+------+------+------+------+
# | DATE_TIME   | POP  | PTY  | REH  | SKY  | T3H  | UUU  | VEC  | VVV  | WSD  |
# +-------------+------+------+------+------+------+------+------+------+------+
# | 05/27 01:05 | 0    | 0    | 95   | 1    | 14   | 0    | 135  | 0.1  | 0.1  |
# +-------------+------+------+------+------+------+------+------+------+------+
...
#            (time.strftime("%m/%d %H:%M"), result['POP'], result['PTY'], result['REH'], result['SKY'], result['T3H'], result['UUU'], result['VEC'], result['VVV'], result['WSD'],))
            (time.strftime("%m/%d %H:%M"), result['POP'], result['PTY'], result['REH'], result['SKY'], result['T3H'], result['UUU'], result['VEC'], result['VVV'], result['WSD'],))
